...
template <int dx, int dy>
int count2(int x, int y, int h) {
    if (h == '9') {
        return 1;
    }

    int result = 0;
    if(dx !=  1 && x > 0          && input[x - 1 + y       * (width + 1)] == h + 1) result += count2<-1,  0>(x - 1, y, h + 1);
    if(dx != -1 && x + 1 < width  && input[x + 1 + y       * (width + 1)] == h + 1) result += count2< 1,  0>(x + 1, y, h + 1);
    if(dy !=  1 && y > 0          && input[x     + (y - 1) * (width + 1)] == h + 1) result += count2< 0, -1>(x, y - 1, h + 1);
    if(dy != -1 && y + 1 < height && input[x     + (y + 1) * (width + 1)] == h + 1) result += count2< 0,  1>(x, y + 1, h + 1);
    return result;
}
